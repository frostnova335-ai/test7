...
class DashboardRestApi(CustomTagsOptimizationMixin, BaseSupersetModelRestApi):
    ...
    @statsd_metrics
    @event_logger.log_this_with_context(
        action=lambda self, *args, **kwargs: f"{self.__class__.__name__}.export_data",
        log_to_statsd=False,
    )
    def export_data(self, id_or_slug: str) -> Response:
        """Export dashboard data in CSV or XLSX format.
        ---
        post:
          summary: Export dashboard data in CSV or XLSX format
          description: >-
            Aggregates data from all charts on the dashboard and returns
            a single CSV or XLSX file for download.
          parameters:
          - in: path
            schema:
              type: string
        ...
        try:
            from superset.commands.dashboard.export_data import (
                ExportDashboardDataCommand,
            )

            buf, filename, mimetype = ExportDashboardDataCommand(
                id_or_slug,
                export_format,
            ).run()
            ...
            logger.error("Dashboard export failed: %s", str(ex), exc_info=True)
