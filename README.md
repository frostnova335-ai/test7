...
console.log("priyanshu ji",queriesData,unitValue,unitLabel)
console.log(queriesData,unitValue)
const rawCustomMetric = formData.custom_unit_metric;
let unitBoxValue: any = null;
console.log(unitBoxValue)
if (rawCustomMetric) {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        const [xIndex, yIndex] = isHorizontal ? [1, 0] : [0, 1];
               const extraTooltipFields = ensureIsArray(formData.extra_tooltip_eld); 
...
if (extraTooltipFields.length && forecastValue?.[0]?.data) {
  extraTooltipFields.forEach(field => {
    const rawVal = (forecastValue[0].data as Record<string, any>)[field]; // Typecast `data` to a dictionary
    const displayValue =
